...
class Organization extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, info, inn, contacts, site, email', 'required'),
			array('name', 'length', 'max' => 500),
			array('info', 'length', 'max' => 1000),
			array('inn', 'length', 'max' => 20),
			array('contacts', 'length', 'max' => 1000),
			array('site, email', 'length', 'max' => 100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, info, inn, contacts, site, email', 'safe', 'on' => 'search'),
		);
	}
